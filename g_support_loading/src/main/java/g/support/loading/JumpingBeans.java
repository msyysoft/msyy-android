...
class JumpingBeans {
    ...
    private static boolean endsWithThreeEllipsisDots(@NonNull CharSequence text) {
        ...
        return text.length() < THREE_DOTS_ELLIPSIS_LENGTH ? false : TextUtils.equals(text.subSequence(text.length() - THREE_DOTS_ELLIPSIS_LENGTH, text.length()), THREE_DOTS_ELLIPSIS);
    }
    ...
    private static CharSequence ensureTextCanJump(int startPos, int endPos, CharSequence text) {
        ...
        if (endPos < startPos) {
            throw new IllegalArgumentException("The start tab_id must be smaller than the end tab_id");
        }

        if (startPos < 0) {
            throw new IndexOutOfBoundsException("The start tab_id must be non-negative");
        }

        if (endPos > text.length()) {
            throw new IndexOutOfBoundsException("The end tab_id must be smaller than the text length");
        }
        return text;
    }
    ...
    public static class Builder {
         ...
         * Call the {@link #build()} method once you're done to get the
         *
         * @param startPos The tab_id of the first character to animate
         * @param endPos   The tab_id after the one the animated range ends at
