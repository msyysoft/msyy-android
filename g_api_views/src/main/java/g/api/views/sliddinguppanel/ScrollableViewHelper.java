...
public class ScrollableViewHelper {
    /**
     * Returns the current scroll tab_id of the scrollable view. If this method returns zero or
     * less, it means at the scrollable view is in a tab_id such as the panel should handle
    ...
    public int getScrollableViewScrollPosition(View scrollableView, boolean isSlidingUp) {
        ...
        if (scrollableView instanceof ScrollView) {
            ...
            if (isSlidingUp) {
                ...
                // Approximate the scroll tab_id based on the top child and the first visible item
                return lv.getFirstVisiblePosition() * firstChild.getHeight() - firstChild.getTop();
            } else {
                View lastChild = lv.getChildAt(lv.getChildCount() - 1);
                // Approximate the scroll tab_id based on the bottom child and the last visible item
