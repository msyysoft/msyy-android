package g.api.views.dynamiclistview.g;
...
import g.api.views.dynamiclistview.DynamicListAdapterInterface;
import g.api.views.dynamiclistview.DynamicListUtils;
import g.api.views.dynamiclistview.DynamicListViewIFaces;
import g.api.views.swipelistview.SwipeMenuAdapter;
 ...
 * Time: 12:31 PM
 */
public class DynamicSwipeMenuListView extends SwipeMenuListView {
    ...
    private DynamicListViewIFaces.OnDropListener mDropListener;
    private DynamicListViewIFaces.OnDragListener mDragListener;
    private DynamicListViewIFaces.OnEditModeChangeListener mEditModeChangeListener;
    ...
    private DynamicListViewIFaces.OnSelectedItemBitmapCreationListener mSelectedItemBitmapCreationListener;
    private View mMobileView;

    /**
     * 冷冻数量：freezePreNum=3代表前面3个item不能移动，freezeLastNum = 4代表后面4个不能移动，默认为0
     */
    private int freezePreNum = 0;
    private int freezeLastNum = 0;


    public DynamicSwipeMenuListView(Context context) {
        super(context);
        init(context);
    }

    public DynamicSwipeMenuListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public DynamicSwipeMenuListView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setOnDropListener(DynamicListViewIFaces.OnDropListener dropListener) {
        this.mDropListener = dropListener;
    }

    public void setOnDragListener(DynamicListViewIFaces.OnDragListener dragListener) {
    ...
    /**
     * Start edit mode with tab_id. Useful for start edit mode in
    ...
    public void startEditMode(int position) {
        ...
        boolean p = freezePreNum < size && position >= freezePreNum;
        boolean l = freezeLastNum < size && position < (size - freezeLastNum);
        if (!(p&&l))
            return;
    ...
    public void setOnEditModeChangeListener(DynamicListViewIFaces.OnEditModeChangeListener editModeChangeListener) {
    ...
    public void setOnSelectedItemBitmapCreationListener(DynamicListViewIFaces.OnSelectedItemBitmapCreationListener selectedItemBitmapCreationListener) {
    ...
    private DynamicListAdapterInterface getAdapterInterface() {
        SwipeMenuAdapter adapter = (SwipeMenuAdapter) getAdapter();
        return ((DynamicListAdapterInterface) adapter.getWrappedAdapter());
    }
    ...
    /**
     * Retrieves the tab_id in the grid corresponding to <code>itemId</code>
     */
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mCellIsMobile) {
                    mHoverCellCurrentBounds.offsetTo(0, mHoverCellOriginalBounds.top + deltaY + mTotalOffsetY);
            ...
            case MotionEvent.ACTION_POINTER_UP:
                 ...
                 * ends and the hover cell is animated to its corresponding tab_id
                 * in the listview. */
    ...
    private void touchEventsEnded() {
        ...
        if (mobileView != null && (mCellIsMobile || mIsWaitingForScrollFinish)) {
            ...
            mHoverCellCurrentBounds.offsetTo(0, mobileView.getTop());
    ...
    private void handleCellSwitch() {
        ...
        final int deltaXTotal = mHoverCellOriginalBounds.centerX() + deltaX;
        ...
        for (Long id : idList) {
            ...
            if (view != null) {
                ...
                if ((aboveRight(targetColumnRowPair, mobileColumnRowPair)
                    ...
                    float xDiff = Math.abs(DynamicListUtils.getViewX(view) - DynamicListUtils.getViewX(mMobileView));
                    float yDiff = Math.abs(DynamicListUtils.getViewY(view) - DynamicListUtils.getViewY(mMobileView));
        ...
        if (targetView != null) {
            ...
            int size = getAdapter().getCount();
            boolean p = freezePreNum < size && targetPosition >= freezePreNum;
            boolean l = freezeLastNum < size && targetPosition < (size - freezeLastNum);
            if (!(p&&l))
                return;

            final DynamicListAdapterInterface adapter = getAdapterInterface();
            ...
            DynamicListViewIFaces.SwitchCellAnimator switchCellAnimator;
    ...
    private class PreHoneycombCellAnimator implements DynamicListViewIFaces.SwitchCellAnimator {
        private int mDeltaY;
        private int mDeltaX;
    ...
    private class KitKatSwitchCellAnimator implements DynamicListViewIFaces.SwitchCellAnimator {
    ...
    private class LSwitchCellAnimator implements DynamicListViewIFaces.SwitchCellAnimator {
    ...
    private OnScrollListener mScrollListener = new OnScrollListener() {
         ...
         * to return to its correct tab_id after the gridview has entered an idle scroll
         * state.
         */
    ...
    public void setFreezePreNum(int freezePreNum) {
        this.freezePreNum = freezePreNum;
    }

    public void setFreezeLastNum(int freezeLastNum) {
        this.freezeLastNum = freezeLastNum;
    }

    public int getFreezePreNum() {
        return freezePreNum;
    }

    public int getFreezeLastNum() {
        return freezeLastNum;
    }
}
